...
export class SQLiteService {
  ...
  async addUser(id: string, password: string, role: string, introSeen: number, paymentUseCount: number) {
    await this.ensureDBOpen();
    await this.db.run(
      `INSERT INTO users (id, password, role, introSeen, paymentUseCount) VALUES (?, ?, ?, ?, ?)`,
      [id, password, role, introSeen, paymentUseCount]
    );
  }
