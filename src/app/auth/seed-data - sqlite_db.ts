...
export async function seedDatabase(sqliteService: SQLiteService) {
  ...
  if (users.length === 0) {
    ...
    // 7 patients
    for (let i = 1; i <= 7; i++) {
      await sqliteService.addUser(`P10${i}`, 'pass123', 'Patient', 0, 0, 'testuser');
    }
  }
  ...
  if (patientCount < 7) {
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      await sqliteService.addPatientRecord({
        ...
        risk: Number((Math.random() * 0.8 * 100 + 0.1).toFixed(2)), // risk between 0.1 and 0.9
  ...
  await sqliteService.saveComments("D101", "Continue medication","Blood pressure improving",  "P101", "Hypertension", "2025-08-01");
  await sqliteService.saveComments("D101", "Suggest lifestyle changes","Patient has mild symptoms",  "P102", "Asthma", "2025-08-02");
  await sqliteService.saveComments("D102", "Keep monitoring diet","Glucose levels stable",  "P105", "Diabetes", "2025-08-03");
